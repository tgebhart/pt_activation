...
class CFF(nn.Module):
    def __init__(self, filters=5, kernel_size=5, fc1_size=50):
        ...
        self.params = [self.conv1,self.fc1,self.fc2]

    def forward(self, x, hiddens=False):
        h1_m = F.sigmoid(self.conv1(x))
        h1 = h1_m.view(-1, (28-self.kernel_size+1)**2*self.filters)
        h2 = F.sigmoid(self.fc1(h1))
        y = self.fc2(h2)
        if hiddens:
    ...
    def compute_dynamic_filtration_no_inf(self, x, hiddens, percentile=0, return_nm=False):
        id = 0
        f = dion.Filtration()
        ...
        for c in range(num_channels):
            p = params[l].weight.data[:,c,:,:]
            mat = conv_layer_as_matrix(p, x[c], stride)
            m1, h0_births, h1_births = conv_filtration_fast2(x[c], mat, l, c, nlc, percentile=percentiles[l])
            # enums = m1
            # enums += [([spec_hash((l,c,i[0]))], h0_births[i].item()) for i in np.argwhere(h0_births > percentile)]
            enums = []
        ...
        percentiles[l] = np.percentile(np.absolute(hiddens[l].cpu().detach().numpy()), percentile)
        p = params[l]
        m1, h0_births, h1_births_2 = linear_filtration_fast2(h1, p, l, 0, percentile=percentiles[l])
        enums += m1
    ...
    if args.dataset == 'fashion':
        train_loader = torch.utils.data.DataLoader(
            datasets.FashionMNIST('../data/fashion', train=True, download=True, transform=transforms.Compose([transforms.ToTensor(),
                               transforms.Normalize((0.5,0.5,0.5), (0.5,0.5,0.5))])), batch_size=args.batch_size, shuffle=True, **kwargs)

        test_loader = torch.utils.data.DataLoader(
            datasets.FashionMNIST('../data/fashion', train=False, download=True, transform=transforms.Compose([transforms.ToTensor(),
                               transforms.Normalize((0.5,0.5,0.5), (0.5,0.5,0.5))])), batch_size=args.test_batch_size, shuffle=False, **kwargs)
